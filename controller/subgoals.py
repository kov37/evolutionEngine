...
def _subgoal_ledger(run_dir: str) -> dict:
...
def has_real_progress_since(run_dir: str, since_event_id: str, exclude_tools=frozenset()) -> bool:
    """Any successful (non-ERROR/REJECTED) tool call after since_event_id,
    excluding bookkeeping calls that don't themselves represent progress on
    the underlying task. Shared by subgoal completion gating and (Phase 4
    item 5) the cross-subgoal stagnation detector."""
...
def make_subgoal_tools(run_store):
    ...
    def subgoal_create(goal: str, success_condition: str) -> str:
        ...
        """
        ledger = _subgoal_ledger(run_store.run_dir)
    ...
    def subgoal_complete(subgoal_id: str, conclusion: str, evidence_ids: str = "") -> str:
        ...
        if not has_real_progress_since(run_store.run_dir, entry["created_event_id"], exclude_tools=BOOKKEEPING_TOOLS):
            return (
